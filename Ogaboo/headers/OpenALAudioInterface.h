...
class OpenALAudioInterface : public TheoraAudioInterface, TheoraTimer
{
	int mMaxBuffSize;
	int mBuffSize;
	short *mTempBuffer;
...
public:
	OpenALAudioInterface(TheoraVideoClip* owner,int nChannels,int freq);
...
{
public:
	OpenAL_AudioInterfaceFactory(const ALCdevice* sndDevive);
	~OpenAL_AudioInterfaceFactory();
	OpenALAudioInterface* createInstance(TheoraVideoClip* owner,int nChannels,int freq);
};
