...
OpenALAudioInterface::OpenALAudioInterface(TheoraVideoClip* owner,int nChannels,int freq) :
...
float OpenALAudioInterface::getQueuedAudioSize()
{
	return ((float) (mNumProcessedSamples - mNumPlayedSamples)) / mFreq;
}

void OpenALAudioInterface::insertData(float* data, int nSamples)
{
...
void OpenALAudioInterface::update(float time_increase)
{
...
void OpenALAudioInterface::pause()
{
	alSourcePause(mSource);
	TheoraTimer::pause();
}

void OpenALAudioInterface::play()
{
	alSourcePlay(mSource);
	TheoraTimer::play();
}

void OpenALAudioInterface::seek(float time)
{
	OpenAL_Buffer buff;
...
OpenAL_AudioInterfaceFactory::OpenAL_AudioInterfaceFactory(const ALCdevice* sndDevive)
{
	...
	gDevice = const_cast<ALCdevice *> (sndDevive);//alcOpenDevice("");
...
OpenALAudioInterface* OpenAL_AudioInterfaceFactory::createInstance(TheoraVideoClip* owner,int nChannels,int freq)
{
	return new OpenALAudioInterface(owner,nChannels,freq);
}
